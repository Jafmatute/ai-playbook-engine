...
import {
  ...
  type UpdatePlaybookSourceConfigurationReferenceCommand,
...
function validCommand(): UpdatePlaybookSourceConfigurationReferenceCommand {
  return Object.freeze({
    playbookSourceId: '00000000-0000-0000-0000-000000000003',
    configurationReference: 'config-updated',
  });
}
...
describe('UpdatePlaybookSourceConfigurationReferenceHandler', () => {
  it('rejects an invalid identifier before making calls', async () => {
    ...
    const handler = new UpdatePlaybookSourceConfigurationReferenceHandler(
      provider,
      workspaceRepo,
      sourceRepo,
    );
    ...
    const result = await handler.handle({
      ...
      configurationReference: 'new-config',
    });
  ...
  it('rejects an empty configuration reference before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
    ...
    expect(result.error.code).toBe('PLAYBOOK_SOURCE_CONFIGURATION_REFERENCE_INVALID');
  ...
  it('rejects an overly long configuration reference before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
    ...
    const result = await handler.handle({
      playbookSourceId: '00000000-0000-0000-0000-000000000003',
      configurationReference: 'x'.repeat(513),
    });
  ...
  it('rejects a configuration reference with control characters before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
  ...
  it('returns PLAYBOOK_SOURCE_UPDATE_INVALID when configuration reference is unchanged', async () => {
    const source = createEnabledSource();
  ...
  it('updates configuration reference on an enabled source with sync history and returns complete DTO', async () => {
    ...
    expect(result.value.configurationReference).toBe('config-updated');
  ...
  it('updates configuration reference on a disabled source', async () => {
    ...
    const result = await handler.handle({
      playbookSourceId: '00000000-0000-0000-0000-000000000003',
      configurationReference: 'config-disabled-updated',
    });
    ...
    expect(result.value.configurationReference).toBe('config-disabled-updated');
    expect(result.value.externalRootReference).toBe(snapshotBefore.externalRootReference);
  });
  ...
  it('does not depend on PlaybookRepository', () => {
    ...
    expect(handler).toBeInstanceOf(UpdatePlaybookSourceConfigurationReferenceHandler);
  });
});
