...
import {
  ...
  type UpdatePlaybookSourceExternalRootReferenceCommand,
...
function validCommand(): UpdatePlaybookSourceExternalRootReferenceCommand {
  return Object.freeze({
    playbookSourceId: '00000000-0000-0000-0000-000000000003',
    externalRootReference: 'https://example.com/updated-root',
  });
}
...
describe('UpdatePlaybookSourceExternalRootReferenceHandler', () => {
  it('rejects an invalid identifier before making calls', async () => {
    ...
    const handler = new UpdatePlaybookSourceExternalRootReferenceHandler(
      provider,
      workspaceRepo,
      sourceRepo,
    );
    ...
    const result = await handler.handle({
      playbookSourceId: 'not-a-uuid',
      externalRootReference: 'new-root',
    });
  ...
  it('rejects an empty external root reference before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
    ...
    expect(result.error.code).toBe('PLAYBOOK_SOURCE_EXTERNAL_ROOT_REFERENCE_INVALID');
  ...
  it('rejects an overly long external root reference before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
    ...
    const result = await handler.handle({
      playbookSourceId: '00000000-0000-0000-0000-000000000003',
      externalRootReference: 'x'.repeat(513),
    });
  ...
  it('rejects an external root reference with control characters before workspace resolution', async () => {
    const provider = new StubCurrentWorkspaceProvider({
      kind: 'workspaceId',
      workspaceId: workspaceIdValue,
    });
  ...
  it('returns PLAYBOOK_SOURCE_UPDATE_INVALID when external root reference is unchanged', async () => {
    const source = createEnabledSource();
  ...
  it('updates external root reference on an enabled source with sync history and returns complete DTO', async () => {
    ...
    sourceRepo.setUpdateResult(ok(valueFrom(PersistenceRevision.from(8))));
    const handler = new UpdatePlaybookSourceExternalRootReferenceHandler(
      provider,
      workspaceRepo,
      sourceRepo,
    ...
    expect(result.value.externalRootReference).toBe('notion-root-updated');
  ...
  it('updates external root reference on a disabled source', async () => {
    ...
    const result = await handler.handle({
      playbookSourceId: '00000000-0000-0000-0000-000000000003',
      externalRootReference: 'notion-root-disabled-updated',
    });
    ...
    expect(result.value.externalRootReference).toBe('notion-root-disabled-updated');
    expect(result.value.configurationReference).toBe(snapshotBefore.configurationReference);
  });
  ...
  it('does not depend on PlaybookRepository', () => {
    ...
    expect(handler).toBeInstanceOf(UpdatePlaybookSourceExternalRootReferenceHandler);
  });
});
