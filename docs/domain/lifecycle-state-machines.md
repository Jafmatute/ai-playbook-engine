...
* No completed snapshot is associated.
* Partial retrieval data must not become an executable snapshot.
...
* Run has not started.
* Cancellation is authorized by the application layer.

Required transition data:

* Cancellation timestamp.
* Cancellation reason or source.
...
* Completion has not already occurred.
...
The version passed all blocking validations and is eligible for execution.
...
* Archived → Published.
* Archived → Draft.
* Published content mutation.
* Invalid content correction in place.

## Correction Rule

An Invalid or Published Playbook Version must not be repaired by mutating its normalized content.
...
If future requirements demand multiple validation attempts over exactly the same immutable content, validation attempts may become separate records without changing the version content.

## Idempotency Rule

Publishing an already Published version with identical validation and publication data may be idempotent.
...
A Draft, Validating, Invalid or Archived version must not be selected for a normal Execution.
...
* Is `Validating` necessary as persisted state, or can validation be represented as a separate process?
* Does Playbook Version need a `ReadyForValidation` state?
* Should `Published` versions be directly executable when not active?
* Can an Archived version be used through an explicit historical replay operation?
* Is Synchronization cancellation required in version 1?
