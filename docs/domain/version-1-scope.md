...
* Register one Notion source for a Playbook.
...
* Version 1 supports one active Notion source per Playbook.
* Additional sources may be registered later but must not contribute to the same version in version 1.
* A Playbook Version is produced from exactly one Synchronization Snapshot.
...
* Completed.
* Failed.
* Cancelled only if cancellation can be implemented without unnecessary complexity.

Cancellation may be deferred from the first implementation without changing the rest of the lifecycle.
...
* Publish the version when all blocking validation passes.
* Archive a Published or Invalid version.
...
12. Valid versions can be published and activated.
